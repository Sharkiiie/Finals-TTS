...
import { type ParsedMessage } from './bot.d'

// Yoinked from Twitch docs and lazily 'converted' to ts. (Got rid of errors lmao)
...
function parseMessage (message: string) {
  const parsedMessage: ParsedMessage | null = {
    tags: null,
    source: null,
    command: {
      botCommand: '',
  ...
  let rawTagsComponent: string | undefined
  let rawSourceComponent: string | undefined
  let rawCommandComponent: string | undefined
  let rawParametersComponent: string | string[] = ''
...
function parseTags (tags: string) {
  ...
  const dictParsedTags: Record<string, string | object> = {} // Holds the parsed list of tags.
  // The key is the tag's name (e.g., color).
  const parsedTags = tags.split(';')

  parsedTags.forEach(tag => {
    const parsedTag = tag.split('=') // Tags are key/value pairs.
    const tagValue = (parsedTag[1] === '') ? '' : parsedTag[1]
    ...
    switch (parsedTag[0]) { // Switch on tag name
      ...
      //     if (tagValue) {
      //         let dict: object = {} // Holds the list of badge objects.
      ...
      //     if (tagValue) {
      //         let dictEmotes: object = {} // Holds a list of emote objects.
      //         // The key is the emote's ID.
      //         let emotes = tagValue.split('/');
      //         emotes.forEach(emote => {
      //             let emoteParts = emote.split(':');

      //             let textPositions: { startPosition: string; endPosition: string; }[] = []
...
function parseSource (rawSourceComponent: string | null | undefined) {
...
function parseParameters (rawParametersComponent: string, command: { botCommand?: string, command?: string | null, botCommandParams?: string }) {
  const idx = 0
